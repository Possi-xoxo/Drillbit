...
from PySide6.QtGui import QColor, QIcon, QPixmap, QKeySequence
from PySide6.QtWidgets import (QCheckBox,QComboBox,QHBoxLayout,QInputDialog,QLabel,QLineEdit,QListWidget,QListWidgetItem,
...
class EditorPanel(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        self.tool=QComboBox();self.tool.addItems(["Pencil","Eyedropper","Flood Fill"]);self.tool.setToolTip("Choose how clicks affect logical diamond cells.")
        self.tool.setItemData(0,"Paint the selected DMC color; drag for one undoable stroke.",Qt.ItemDataRole.ToolTipRole)
        self.tool.setItemData(1,"Pick the DMC color from any pattern cell.",Qt.ItemDataRole.ToolTipRole)
        self.tool.setItemData(2,"Replace one four-directionally connected color region.",Qt.ItemDataRole.ToolTipRole)
        self.undo=QPushButton("Undo");self.undo.setShortcut(QKeySequence.StandardKey.Undo);self.redo=QPushButton("Redo");self.redo.setShortcut(QKeySequence.StandardKey.Redo)
        ...
        for widget in (QLabel("Tool"),self.tool,self.undo,self.redo,self.highlight,self.before):tools.addWidget(widget)
        ...
        self.tool.currentTextChanged.connect(lambda value:setattr(self.canvas,"tool",value));self.search.textChanged.connect(self._populate_palette)
    ...
    def set_pattern(self,pattern):
        self.pattern=pattern;self.undo_stack=UndoStack();self.canvas.set_pattern(pattern,self.undo_stack);self._populate_palette();self._refresh_used();self.select_code(next(iter(pattern.usage)));self._buttons()
    ...
    def _refresh_used(self):
        self.used_list.clear();total=self.pattern.width*self.pattern.height
        ...
        summary=region_summary(self.pattern);self.analysis.setText(f"Single-cell regions: {summary['single_cell_regions']:,} | Regions of 3 or less: {summary['regions_le_3']:,}")
    ...
    def _buttons(self):self.undo.setEnabled(self.undo_stack.can_undo);self.redo.setEnabled(self.undo_stack.can_redo)
    def _undo(self):
