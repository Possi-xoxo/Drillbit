...
def render_reference(logical: Image.Image, cell_size: int = 10, show_grid: bool = False) -> Image.Image:
    ...
    result = logical.resize((logical.width * cell_size, logical.height * cell_size), Image.Resampling.NEAREST)
    if show_grid and cell_size > 1:
        draw = ImageDraw.Draw(result)
        line=(80,80,80,255) if result.mode=="RGBA" else (80,80,80)
        for x in range(0, result.width, cell_size):
            draw.line((x, 0, x, result.height - 1), fill=line)
        for y in range(0, result.height, cell_size):
            draw.line((0, y, result.width - 1, y), fill=line)
    return result
