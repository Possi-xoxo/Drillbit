...
class PatternModel:
    def __init__(self,width,height,cell_ids,palette,metadata=None,initial_ids=None):
        ...
        invalid=set(cell_ids)-set(palette.by_code)
        ...
        self.initial_ids=list(initial_ids if initial_ids is not None else cell_ids); self.usage=Counter(self.cell_ids)
    ...
    def _set_index(self,index,code):
        if code not in self.palette.by_code: raise ValueError(f"Unknown palette code {code}")
        old=self.cell_ids[index]
        if old==code:return None
        self.cell_ids[index]=code; self.usage[old]-=1
    ...
    def to_image(self,initial=False):
        ...
        image=Image.new("RGB",(self.width,self.height)); image.putdata([self.palette.by_code[code].rgb for code in ids]); return image
...
class UndoStack:
    def __init__(self): self.commands=[]; self.position=0
    @property
    ...
    def push(self,label,changes):
        ...
        self.commands=self.commands[:self.position];self.commands.append(EditCommand(label,changes));self.position+=1
    def undo(self,pattern):
        if not self.can_undo:return False
        self.position-=1;pattern.apply_changes(reversed(self.commands[self.position].changes),False);return True
    def redo(self,pattern):
        if not self.can_redo:return False
        pattern.apply_changes(self.commands[self.position].changes,True);self.position+=1;return True
